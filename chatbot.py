...
# MongoDB Configuration - Updated for your database
mongo_client = MongoClient(os.getenv("MONGODB_URI"))
db = mongo_client["mongo"]  # Your specified database name
chat_collection = db["chat"]  # Your specified collection name
...
def create_collections():
    ...
    chat_collection.create_index([("session_id", 1), ("role", 1)])

def generate_session_id():
    """Generate a unique session ID and initialize in MongoDB"""
...
def store_message(session_id, role, message):
    """Store message in both MongoDB and vector store"""
    # Store in MongoDB
    message_data = {
    ...
    # Store in Qdrant (only for user messages)
    if role == "user":
        ...
        # Clean up old messages in Qdrant
...
def get_verified_context(session_id):
    """Retrieve messages containing numerical data from MongoDB"""
    pipeline = [
        ...
        {"$match": {"has_numbers": True}},
        {"$sort": {"timestamp": -1}},
        {"$limit": 10}
    ]
    
    messages = list(chat_collection.aggregate(pipeline))
...
def chat_with_session(session_id, user_message):
    ...
    try:
        ...
        return {"answer": "❌ Invalid session ID format. Please generate a valid session."}

    # Verify session exists (check MongoDB)
    if not chat_collection.find_one({"_id": session_id}):
        return {"answer": "❌ Session not found. Please start a new session."}
    ...
    try:
        ...
        return {"answer": "Sorry, I couldn't generate a response at this time."}
